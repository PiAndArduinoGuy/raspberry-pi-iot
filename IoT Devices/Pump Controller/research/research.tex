...
\begin{document}
	...
	\label{sec:mcp3002}
	Image added to illustrate pip package to read MCP3002. Talk about how we can create a python script that we can use to read temp for time being (until atleast we get java code from pi4j than can read MCP3002 ADC values).
	
	% section mcp3002 (end)

	\section{Earthing} % (fold)
	\label{sec:earthing}
	Earthing of GND connection on pi zero to solve hanging voltage on ground line that results in relay not toggling
